...
{
    public interface IAppChatCompletionService
    {
        Task RunAsync();
        IAsyncEnumerable<ChatMessageContent> CallChatCompletionAgent(string instructions = "", string prompt = "");
    }

    public class AppChatCompletionService : IAppChatCompletionService
    {
        private readonly Kernel _kernel;
        private readonly ILogger<AppChatCompletionService> _logger;

        public AppChatCompletionService(Kernel kernel, ILogger<AppChatCompletionService> logger)
        {
            _kernel = kernel;
            _logger = logger;
        }
        ...
        public async IAsyncEnumerable<ChatMessageContent> CallChatCompletionAgent(string instructions = "", string prompt = "")
        {
            if (string.IsNullOrEmpty(instructions)) instructions = "You are a helpful assistant.";
            if (string.IsNullOrEmpty(prompt)) prompt = "What is the price of the soup special?";
